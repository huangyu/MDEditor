...
public class MixtureTextView extends RelativeLayout {
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        ...
        mTextSize = typedArray.getDimensionPixelSize(INDEX_ATTR_TEXT_SIZE, mTextSize);
        mTextColor = typedArray.getColor(INDEX_ATTR_TEXT_COLOR, mTextColor);
        mText = typedArray.getString(INDEX_ATTR_TEXT);
        typedArray.recycle();
    }
    ...
    private List<Rect> caculateViewYBetween(int y1, int y2) {
        ...
        if (rs.size() >= 2) {
            List<Rect> res = new ArrayList<>(rs);
